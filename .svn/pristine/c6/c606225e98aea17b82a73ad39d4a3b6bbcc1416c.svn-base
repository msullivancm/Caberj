...
user function nfseXMLEnv( cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela,aAIDF )
	...
	Private lMvded		:= SuperGetMV("MV_NFSEDED",,.T.)	// Habilita/Desabilita as Deducoes da NFSE
	...
	//----------------------------------------------------------------
	//Angelo Henrique - Data: 08/10/2020
	//Inicio - Ajuste Release 27 - P12 - Data: 08/10/2020
	//----------------------------------------------------------------
	/*
	...
	//----------------------------------------------------------------
	//Angelo Henrique - Data: 08/10/2020
	//Fim - Ajuste Release 27 - P12 - Data: 08/10/2020
	//----------------------------------------------------------------
	...
	If cTipo == "1" .And. Empty(cMotCancela) 
		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
			...
			Endif
			//*****************************************************************************
			// caso o F2_HORA não esteja preenchido - será enviado a hora da execução
			// 20/06/2018 - Mateus
			//***************************************************************************** 
			if !empty(AllTrim(SF2->F2_HORA)) 
			...
			If !SF2->F2_TIPO $ "DB" 
				...
				If !Upper(SA1->A1_EST) == "EX"
					...
					//aadd(aDest, iIf(cFilAnt=='04', '3303302', SA1->A1_COD_MUN ))
				...
				If SF2->(FieldPos("F2_CLIENT"))<>0 .And. !Empty(SF2->F2_CLIENT+SF2->F2_LOJENT) .And. SF2->F2_CLIENT+SF2->F2_LOJENT<>SF2->F2_CLIENTE+SF2->F2_LOJA
					...
					aadd(aEntrega,SA1->A1_COD_MUN /*iIf(cFilAnt=='04', '3303302', SA1->A1_COD_MUN )*/)
           ...
           If cMvMsgTrib $ "1-3"
				...
				BeginSql Alias cAliasSD2
					SELECT D2_FILIAL,D2_SERIE,D2_DOC,D2_CLIENTE,D2_LOJA,D2_COD,D2_TES,D2_NFORI,D2_SERIORI,D2_ITEMORI,D2_TIPO,D2_ITEM,D2_CF,
						...
						D2_VALPIS,D2_VALCOF,D2_VALCSL,D2_VALIRRF,D2_VALINS,D2_ORIGLAN,D2_VALICM	,
						 D2_XPREPLS  , D2_XTITPLS // Customização -  Inclusão de campos para compatibilização com a BM1 - 20/06/2018					
					FROM %Table:SD2% SD2
					WHERE
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If IntTms()
					...
					If DT6->(DbSeek(xFilial("DT6")+SF2->(F2_FILIAL+F2_DOC+F2_SERIE)))
						...
						Else
							SA1->(DbSetOrder(1))
							If SA1->(DbSeek(xFilial("SA1")+DT6->(DT6_CLIDES+DT6_LOJDES)))
								cMunPrest := SA1->A1_COD_MUN //iIf(cFilAnt=='04', '3303302', SA1->A1_COD_MUN )
							EndIf
						EndIf					
					Else
					...
					Else
						...
						Else
							...
							Elseif ((cAliasSD2)->D2_ORIGLAN $ "VD")
								...
								EndIf
					   		Else
								cMunPrest := aDest[07] /*IIF(cFilant=="04",'3303302',aDest[07])*/
							Endif
				...
				// 20/06/2018 - 		
				// Customização -  de query para buscar os valores dos lotes 
				If cEmpAnt == '01'     
				
	                cSql := " SELECT BM1_ANO, BM1_MES, DECODE(BM1_CODTIP, '101', 'CONTRAPRESTACAO', 'OUTROS') SQLDESCR , SUM( decode(bm1_tipo,1, BM1_VALOR,2,BM1_VALOR*-1)) SQLVALOR   " 
	                cSql += " FROM  "+RetSqlName("BM1")+" BM1 "  
	                
	                cSql += "   INNER JOIN " + RETSQLNAME("BFQ") + " BFQ ON " 
					cSql += "       BFQ_FILIAL = ' '   " 
					cSql += "       AND BFQ_PROPRI||BFQ_CODLAN = BM1_CODTIP " 
				//	cSql += "       AND ( ( BM1_NIVCOB = '4' AND BFQ_YTPANL <> 'C') OR BM1_NIVCOB <> '4' ) "
					cSql += "       AND BFQ.D_E_L_E_T_ = ' '  " 
	
			        cSql += " WHERE BM1_FILIAL = '"+xFilial("BM1")+"' "  
		 		    cSql += " AND BM1_PREFIX = '"+(cAliasSD2)->D2_XPREPLS +"' "
				    cSql += " AND BM1_NUMTIT = '"+(cAliasSD2)->D2_XTITPLS +"' "
				    cSql += " and BM1.d_e_L_e_t_ = ' '  "
				    // cSql += " AND BM1_CODTIP <> '903'  "
		            cSql += " group by BM1_ANO, BM1_MES, DECODE(BM1_CODTIP, '101', 'CONTRAPRESTACAO', 'OUTROS') " 
			    	       
                Else 

	                cSql := "SELECT  BM1_ANO, BM1_MES, BM1_DESTIP SQLDESCR , SUM( decode(bm1_tipo,1, BM1_VALOR,2,BM1_VALOR*-1)) SQLVALOR " 
	                cSql += " FROM  "+RetSqlName("BM1")+" BM1 "  
				    ...
				    cSql += " AND BM1_NUMTIT = '"+(cAliasSD2)->D2_XTITPLS +"' "
				    cSql += " and BM1.d_e_L_e_t_ = ' '  "
				    //cSql += " AND BM1_CODTIP <> '903'  " //Angelo Henrique - Data: 17/09/2019
				    cSql += " AND BM1_CODTIP NOT IN ('903','920','907')  "
		            cSql += " group  by BM1_ANO, BM1_MES, BM1_DESTIP " 
                ...
                dbSelectArea("TMP")
                tmp->(dbGoTop())                                         
                //cNatOper += iif(cFilAnt=='04',';', '') + CRLF    
                n_Val_Tot := 0                                        
                While !TMP->(EOF())                    
                cNatOper += CRLF +strtran(TMP->SQLDESCR,'ª','')+ "   " + TRANSFORM(noround(TMP->SQLVALOR,2),"@E 999,999,999.99")   + iif(cFilAnt=='04',';', '') + CRLF       
                    n_Val_Tot += TMP->SQLVALOR
              //  Transform(noround(val(strtran(strtran(TMP->SQLVALOR,'.',''),',','.')),2), "@E 999.999.999,99")                                                         
	                  dbSkip()   // Avanca o ponteiro do registro no arquivo
 	            EndDo                                            
 	            
 	            tmp->(dbGoTop())
 	            
                cNatOper +=  CRLF +  CRLF +  UPPER(MESEXTENSO(TMP->BM1_MES)) + "/" + TMP->BM1_ANO  + iif(cFilAnt=='04',';', '')  

                cNatOper +=  CRLF +  CRLF +  (cAliasSD2)->D2_SERIE + (cAliasSD2)->D2_DOC + iif(cFilAnt=='04',';', '') 

                cNatOper +=  CRLF +  CRLF +  "VAL APROX DOS TRIBUTOS R$: " + Transform((n_Val_Tot*0.163), "@E 999,999.99") + " (16.3%) ."  + iif(cFilAnt=='04',';', '') 
...
static function prestacao( cMunPrest, cDescMunP, aDest, cMunPSIAFI )
	...
	if !Empty( allTrim (cMvNFSEINC) ) 
		...
		Endif		
	Else
		// Mateus Medeiros - 03/09/2018 
		if cEmpAnt == "02" .and. cFilant == "04" .and. alltrim(cMunPrest) <> '3303302'
			cString	+= "<codmunibgeinc>3303302</codmunibgeinc>"
		else 
			if !empty( allTrim (cMunPrest) )
...
static function myGetEnd( cEndereco, cAlias )
	...
	if ( &( cAlias + "->" + cCmpEst ) == "DF" ) .Or. ( ( cAlias )->( FieldPos( cCmpEndN ) ) > 0 .And. &( cAlias + "->" + cCmpEndN ) == "1" )
		aRet[1] := strtran(cEndereco,'º',' ')
		aRet[3] := "SN"
	else
		aRet := fisGetEnd( strtran(cEndereco,'º',' ') )
	endIf
