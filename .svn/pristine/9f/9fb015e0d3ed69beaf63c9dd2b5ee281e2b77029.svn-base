...
/*/{Protheus.doc} CABA634
Lista parâmetros ao usuário com as opções de incluir, alterar e excluir banco digital
...
@version 1.0
    ...
    @param lInclui, Lógico, Define se será possível a inclusão de parâmetros
    @example
    aParams := {"MV_BCODIG"}
    u_AltSX6(aParams, .T., .T.)
/*/

User Function CABA634()
...
*/
    ...
    AltSX6(aParams, .t., .f., .t.)                                                                                              
Return                    

Static Function AltSX6(aParams, lCombo, lDelet, lInclui)
    Local aArea   := GetArea()
    ...
    //Botões
    Private aButtons    := {}
    //Por default lInclui e lDelet são falsos para evitar atividades indevidas do usuário
    If lInclui
...
Static Function fAtuaCols(lFirst)
    ...
    While !SX6->(EoF())
        ...
        //If Alltrim(SX6->X6_VAR) $ cParamsPvt
            If Alltrim(SX6->X6_VAR) = aParams[nI]
