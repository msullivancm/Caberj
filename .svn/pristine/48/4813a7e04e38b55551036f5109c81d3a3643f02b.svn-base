...
±±ºPrograma  ³ CABA090    º Autor ³ Angelo Henrique    º Data ³  10/01/19 º±±
...
User Function CABA090()
	...
	oBtn2      := TButton():New( 044,084,"Gerar Relatorio"	,oGrp1,{||u_CABA090A()	},060,012,,,,.T.,,"",,,,.F. )
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³ CABA090A   º Autor ³ Angelo Henrique    º Data ³  10/01/19 º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
User Function CABA090A
	...
	If Pergunte(cPerg,.T.)
		...
		cQry += "    (                                                						" 	+ CRLF
		cQry += "        SELECT                                       						" 	+ CRLF
		cQry += "            MAX(TO_DATE(DT_IMPOR,'DD/MM/YYYY')) DT_IMPORTA                 " 	+ CRLF
		cQry += "        FROM                                         						" 	+ CRLF
		cQry += "            EMS_IMPORTA                              						" 	+ CRLF
		cQry += "    ) DT_IMPORTA,		                                   					" 	+ CRLF
		cQry += "    (                                                						" 	+ CRLF
		cQry += "        SELECT                                       						" 	+ CRLF
		...
		cQry += "            EMS_IMPORTA                              						" 	+ CRLF
		cQry += "        WHERE                                        						" 	+ CRLF
		cQry += "            TO_DATE(DT_IMPOR,'DD/MM/YYYY') = (                             " 	+ CRLF
		cQry += "                            SELECT                   						" 	+ CRLF
		cQry += "                            	MAX(TO_DATE(INTERNA.DT_IMPOR,'DD/MM/YYYY'))	" 	+ CRLF
		cQry += "                            FROM 											" 	+ CRLF
		...
		If !((cAliQry)->(EOF()))
			...
			_cParams:= AllTrim(cValToChar(MV_PAR01)) + ";" + AllTrim(DTOC((cAliQry)->DT_IMPORTA)) + ";" + AllTrim((cAliQry)->HORA)  
