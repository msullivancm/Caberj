...
private aBrwPEG
Private aCabPEG		:= {" ", " ","Rda","Grp Pag","Vlr Fat.Amb","Vlr Fat.Hosp","Vlr 31/2 Amb","Vlr 31/2 Hosp", "Vlr Não liberado" , "Vlr Total" , "Vlr Previa", "Lib/Pend" , "Nvl"}
Private aTamPEG		:= {15,15,100,120,45,45,45,45,45,45,45,40,10}
...
If nCont := cVlrPgto <= cVlrPrev  
   ...
   Else                                                                       
      ...
      cQryLin +="            AND  BD7.BD7_ANOPAG || BD7.BD7_MESPAG >= '201412' AND  BD7.BD7_ANOPAG || BD7.BD7_MESPAG <= '"+mv_par01+mv_par02+"' " + CRLF 
...
If mv_par07 == 1 
   ...
   cQryPEG +="            AND BD7.BD7_ANOPAG || BD7.BD7_MESPAG <= '"+mv_par01+mv_par02+"' " + CRLF
Else   
   cQryPEG +="            AND BD7.BD7_ANOPAG BETWEEN '2014' AND '" +mv_par01+ "' " + CRLF     
   cQryPEG +="            AND  BD7.BD7_ANOPAG || BD7.BD7_MESPAG >= '201412' " + CRLF
   cQryPEG +="            AND  BD7.BD7_ANOPAG || BD7.BD7_MESPAG <= '"+mv_par01+mv_par02+"' " + CRLF 
EndIf 
cQryPEG +="            AND BD6_FILIAL = '" + xFilial('BD6') + "'  AND BD6.D_E_L_E_T_ = ' ' " + CRLF
cQryPEG +="            AND B16_FILIAL = '" + xFilial('B16') + "'  AND B16.D_E_L_E_T_ = ' ' " + CRLF
...
If mv_par11 == 1 .or.  mv_par06 <= 3 
   ...
   If mv_par11 == 1 
      ...
      cQryPEG +="        or ( SUM(TAB.VALOR_LOTE) - SUM(DECODE (TAB.SEQ, '3',TAB.VALOR_LOTE, 0.00)) > sum(nvl(ZZP.VLRTOT, 0.00)) and APR.AL_NIVEL <= '06') )   " + CRLF 
      
      cQryPEG +="       AND  SUM (DECODE (TAB.SEQ, '3',TAB.VALOR_LOTE, 0.00)) > 0 AND SUM (DECODE (TAB.SEQ, '3',TAB.VALOR_LOTE, 0.00)) <= "+strtran(Transform(Mv_par04,'@E  999999999.99'),',','.')+" " + CRLF 
...
While !cAliasPEG->(EOF())
	...
	aAdd(aRetPEG,{.F. , cAliasPEG->RDA , cAliasPEG->GRPPAG , cAliasPEG->FATURADA_A , cAliasPEG->FATURADA_H , cAliasPEG->ATVPRT35_A , cAliasPEG->ATVPRT35_H , cAliasPEG->ATVPRT35_NL , cAliasPEG->VLR_TOTAL , cAliasPEG->MARCA , cAliasPEG->NIVEL_APR ,cAliasPEG->VLRTOTP , .T. ,cAliasPEG->QTD_ANOMES  })
...
If empty(aRetPEG)
	aAdd(aRetPEG,{.f.,'','',  0 , 0 , 0 , 0 , 0 ,0,'','',0,.T.,''})
 ...
 Private lfaz:= .F.
  ...
  For nI := 1 to len(aBrwPEG)
      If aBrwPEG[nI,1] == .T.
         ...
         Processa({||aBrwDem := aDadosDem(cCodRdaD)},'Processando Demonstrativo ...','buscando dados no servido ...',.T.)
...
next     
    ...
    If cRdaCodA != cRdaCod .and. lfaz == .T.
       ...
       cRdaCodA := cRdaCod	        	
		...
		cQryDem +="                nvl(substr(al_nome,1,15),'Não Avaliado')  Aprovador, decode(bd7_ylibfa,'T','Liberado','Pendente') Lib " + CRLF
   ...
   Else 
    //cQryPEG +="            AND  BD7.BD7_ANOPAG || BD7.BD7_MESPAG <= '"+mv_par01+mv_par02+"' " + CRLF 
      cQryDem +="            AND  BD7.BD7_ANOPAG || BD7.BD7_MESPAG >= '201412' AND  BD7.BD7_ANOPAG || BD7.BD7_MESPAG <= '"+mv_par01+mv_par02+"' " + CRLF 
   EndIf  
   If mv_par10 == 1  
...
//		cQryDem +="            AND BD7_NUMLOT = ' ' " + CRLF     
        ...
        cQryDem +="                 nvl(substr(al_nome,1,15),'Não Avaliado')  , decode(bd7_ylibfa,'T','Liberado','Pendente')" + CRLF        
...
PutSX1(cPerg , "01" , "Ano Competencia " 	,"","","mv_ch1","C",4,0,0,"G",""	,"","","","mv_par01","","","","","","","","","","","","","","","","",aHelp,aHelp,aHelp)

aHelp := {}
aAdd(aHelp, "Informe o Mês da Competencia ")
PutSX1(cPerg , "02" , "Mes Competencia " 	,"","","mv_ch2","C",2,0,0,"G",""	,"","","","mv_par02","","","","","","","","","","","","","","","","",aHelp,aHelp,aHelp)

aAdd(aHelp, "Ordernar Por Rda , Grupo de Pagto ")
PutSX1(cPerg,"03", "Ordernar Por ","","","mv_ch3","N",1,0,1,"C","","","","","mv_par03","RDA"	,"","","","Grupo de Pagto ","","","","","","","","","","","",aHelp,aHelp,aHelp)
 ...
 Private lfaz:= .F.
  ...
  For nI := 1 to len(aBrwDem)
      If aBrwDem[nI,1] == .T.
         ...
         oDlg2  		:= MsDialog():New( aSizeAut[7]-30,00,aSizeAut[3]-320,aSizeAut[5]-70,"Composição do Pagamento ",,,.F.,,,,,,.T.,,,.T. ) 

         oSayLin    	:= TSay():New( aPosObj[1][1],aPosObj[1][2],{||'Composição do pagto  '},oDlg2,,,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,236,016)
...
TcQuery cQryLin New Alias cAliasLin
		...
		While !cAliasLin->(EOF())
	 		...
	 		aAdd(aRetLin,{ cAliasLin->RDA   , cAliasLin->evento ,cAliasLin->locDig  ,cAliasLin->CodPeg , cAliasLin->NumGuia , cAliasLin->origem ,;
    	 			        cAliasLin->Linha , cAliasLin->codunm ,cAliasLin->TABELA  ,cAliasLin->proces , cAliasLin->VlrPgto , cAliasLin->VlrApres,;
    	 			        cAliasLin->VlrGlo ,  cAliasLin->DTPRO  , cAliasLin->MATRIC , cAliasLin->qtdpro , cAliasLin->vlrunit })
