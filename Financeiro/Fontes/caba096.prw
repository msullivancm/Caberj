...
User Function CABA096e(lSchedule)
	...
	conout("INอCIO PROCESSAMENTO - SCHEDULE CABA611 - "+Time())
...
Static Function execAnexo(lSchedule)
	...
	if cEmpAnt == '01'
		cRegAns := '324361'
		conout("INอCIO PROCESSAMENTO - SCHEDULE CABA096E - CABERJ - "+Time())
	else	
		 cRegAns :=  '415774'		
		 conout("INอCIO PROCESSAMENTO - SCHEDULE CABA096E - INTEGRAL - "+Time())
	endif 
...
/*
			...
			If TcSqlExec(cQuery) <> 0	
				conout("Aten็ใo","Erro na execu็ao do envio de boletos.")   
			Else                 
				conout("Aten็ใo","Execu็ใo de envio de boleto finalizada.") 
			EndIf  
...
User Function CABA096G()
	...
	*/
	...
	If TcSqlExec(cQuery) <> 0	
		conout("Aten็ใo","Erro na execu็ao da gera็ใo da Pr้via.")   
	Else                 
		conout("Aten็ใo","Execu็ใo de envio de boleto finalizada.") 
	EndIf  
