...
Static Function RunReport(Cabec1,Cabec2,Titulo,nLin)
	...
	If (SubStr(cNumEmp,1,2) == "01")
		cLogoCli := cPath+"lgrl0101.bmp"
	Else
		cLogoCli := cPath+"lgrl0201.bmp"
	EndIf
	...
	For J:= 1 to nQtd
		...
		If BA3->(MsSeek(xFilial("BA3")+cCodInt+cCodEmp+cMatric))
			...
			If Empty(cCodCli)
				DbSelectArea("BA1")
				dbSetOrder(1)
				dbSeek(xFilial("BA1")+cCodInt+cCodEmp+cMatric)

				DbSelectArea("BQC")
				dbSetOrder(1)	
				BQC->(dbSeek(xFilial("BQC")+cCodInt+cCodEmp+BA1->BA1_CONEMP+BA1->BA1_VERCON+BA1->BA1_SUBCON))
					...
					If Empty(cCodCli)
						DbSelectArea("BT5")
						dbSetOrder(1)	
						dbSeek(xFilial("BT5")+cCodInt+cCodEmp+BA1->BA1_CONEMP)
							cCodCli := BT5->BT5_CODCLI
							cLojCli	:= BQC->BT5_LOJA		
		...
		If ( (!Empty(cCodCli)) .AND. ((!lSoBlq) .OR. (lSoBlq .AND. !Empty(BA3->BA3_DATBLO)))) // 09/03/09
			...
			BA1->(MsSeek(xFilial("BA1")+cCodInt+cCodEmp+cMatric))
			...
			*/
			...
			//------------------------------------------------------------------------
			IF !empty(tmp->cpf) .and. (u_ValDpla(tmp->cpf))  //tmp->Matricula1 == cfamiia
			...
			ElseIf u_ValRetia(BA1->BA1_CODINT, BA1->BA1_CODEMP, BA1->BA1_MATRIC, BA1->BA1_TIPREG, BA1->BA1_DIGITO ) .And. cAno == "2015"
				...
				//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
				oPrn:Line(Eval(bLin,_nLinGr,6), nPasso, _nLinGr, 2400)//cAno
				oPrn:Say (Eval(bLin,_nLinGr,5), nPasso, "IMPOSTO DE RENDA "+cValToChar(cAnoEx)+" - RETIFICAÇÃO DO INFORMATIVO "							,oFont5,100)
				...
				oPrn:Say (Eval(bLin,_nLinGr,2), nPasso, ""+cValToChar(cAnoEx)+",  informamos  os  valores  abaixo  retificados,  incluindo o valor "	,oFont5,200)
				oPrn:Say (Eval(bLin,_nLinGr,2), nPasso, "descontado   em  contracheque  no  mês  de  dezembro/"+cAno+",  competência " 	,oFont5,200)
				oPrn:Say (Eval(bLin,_nLinGr,2), nPasso, "janeiro/"+cValToChar(cAnoEx)+",    referente    à     assistência     médica    repassada " 	,oFont5,200)
				oPrn:Say (Eval(bLin,_nLinGr,2), nPasso, "financeiramente    pela    PREVI/SEPLAG   à   CABERJ    em  05/01/"+cValToChar(cAnoEx)+". "	,oFont5,200)
			...
			Else
				...
				EndIF
				While !TRBUSR2->(Eof())
					//BA1->(MsSeek(xFilial("BA1")+TRBUSR2->CodInt+TRBUSR2->CodEmp+TRBUSR2->Matric))
					DbSelectArea("BA1")
					DbsetOrder(2)
					BA1->(DbSeek(xFilial("BA1")+TRBUSR2->CodInt+TRBUSR2->CodEmp+TRBUSR2->Matric+TRBUSR2->TIPREG))
					//           BA1->(MsSeek(xFilial("BA1")+cCodInt+cCodEmp+cMatric+TRBUSR2->TIPREG))
					...
					ElseIf dtos(BA1->BA1_DATNAS) < '20100101' .and. empty (trim(BA1->BA1_CPFUSR))  .AND. BA1->BA1_tipusu = "D"
						...
						If dtos(BA1->BA1_DATNAS) <'20100101'  .Or. BA1->BA1_tipusu = "T" .or. cAno >= '2019' 
							Ccpf :=BA1->BA1_CPFUSR
						else
							Ccpf := '           '
						EndIf
...
User Function ValRetia(_cCodInt, _cCodEmp, _cMatric, _cTipReg, _cDigit )
