...
User Function FIRSTNFSE()
	...
	//----------------------------------------------------------------------------------------------
	//Rotina responsável por atualizar as notas que já foram transmitidas
	//em alguns casos o protheus informava que a nota não havia sido transmitida, porém
	...
	//----------------------------------------------------------------------------------------------
	//Como estávamos tendo problemas com niteroi foi necessário criar uma rotina para validar
	//o cadastro do cliente e enviar nota a nota, limpando assim forçadamente o XML
	//----------------------------------------------------------------------------------------------
			...
			Else
				...
				If ParamBox(aPerg,"Monitor NFS-e",@aParam,,,,,,,cParMnt,.T.,.T.)
					...
					If !Empty(aListBox)
						...
						@ aPosObj[1,1],aPosObj[1,2] LISTBOX oListBox Fields HEADER "","ID","Ambiente","Modalidade","Protocolo","Cod. Ret",STR0051,STR0052,STR0053; //"NF"###"Ambiente"###"Modalidade"###"Protocolo"###"Recomendação"###"Tempo decorrido"###"Tempo SEF"
...
User Function FIRSTVLD()
	...
	If Pergunte(_cPerg,.T.)
		...
		Aviso("Atenção","Processo Finalizado",{"OK"})
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³VLDQRYNF  ºAutor  ³Angelo Henrique     º Data ³  24/10/16   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³VLDCLINF  ºAutor  ³Angelo Henrique     º Data ³  28/03/19   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
Static Function Fi022Tr(cCodMun,cSerie,cNotaini,cNotaFin,cForca,cAlias,cNotasOk,cDEST,nForca,cMensRet,dDataIni,dDataFim,lAuto,nMaxTrans,cGravaDest,lUsaColab,lNFTE)
	...
	elseif ( SE2->( FieldPos("E2_FIMP") ) == 0 .Or. SE2->( FieldPos("E2_NFELETR") ) == 0 ) .And. cEntSai == "0"
		Aviso("","O campo E2_FIMP ou E2_NFELETR não existem, veiricar se o compatibilizador NFEP11R1 / update NFE11R136 foi executado corretamente!" ,{"Ok"})
	endif		
